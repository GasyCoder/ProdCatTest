...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Ajouter un produit</h1>

    <form action="{{ route('admin.produits.store') }}" method="POST" class="bg-white rounded-lg border p-6" id="product-form">
        @csrf
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-gray-900 focus:ring-gray-900 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 flex justify-end gap-4">
            ...
            <button type="submit"
                    ...
                    id="submit-button">
                <span id="button-text">Ajouter le produit</span>
